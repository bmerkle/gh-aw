...
The compiler uses repository ID comparison for reliable fork detection that is not affected by repository renames. See the [Security Guide](/gh-aw/introduction/architecture/#fork-protection-for-pull-request-triggers) for detailed security implications.
...
See the [Security Guide](/gh-aw/introduction/architecture/#workflow_run-trigger-security) for detailed security behavior and implementation.
